...
class JointBasedAPFPlanner:
    ...
    def plan(self, start_pose, target_pos_3d, obstacle_pos_3d):
        ...
        current_pose = start_pose[:4].copy()
        flag = False
        max_iterations = 30
...
class MobileManipulatorEnv(gym.Env):  
    ...
    def __init__(self, gui=False, fixed_scene=False,complex_scene=False, channel_width=1.2):
    ...
    def setup_robot_joints(self):
        ...
        self.arm_joint_indices = [
            target_joints['shoulder_pan_joint'],   
            target_joints['shoulder_lift_joint'],  
            target_joints['elbow_joint'],          
            target_joints['wrist_1_joint'],        
            target_joints['wrist_2_joint'],        
            target_joints['wrist_3_joint']         
        ]
    ...
    def setup_random_scene(self):
        self.target_pos = [
            np.random.uniform(2.4, 2.5),   
            np.random.uniform(0.01, 0.02),  
            np.random.uniform(1, 1.1)  
        ]

        self.obstacle_pos = [
            np.random.uniform(1.3, 1.5),   
...
if __name__ == "__main__":
    try:
        ...
        print("基于关节势能作用点的6-DOF测试完成，准备关闭...")
        time.sleep(2)
        env.close()
        
    except Exception as e:
        print(f"基于关节势能作用点的6-DOF测试过程中发生错误: {e}")
        import traceback
        traceback.print_exc()